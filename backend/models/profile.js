...
module.exports.retrieveProfileData = function retrieveProfileData(email) {
  const sql = `SELECT a.email, a.name, a.gender, a.phone, a.address, a.region,i.url FROM appuser a JOIN image i ON a.imageid = i.imageid WHERE email = $1`;
...
module.exports.updateProfile = function updateProfile(email, profileData) {
    const updateProfileQuery = `
        UPDATE appuser
        SET name = $1,  gender = $2, phone = $3, address = $4,updatedat = NOW()
        WHERE email = $5
        RETURNING email
    `;

    const { name,  gender, phone, address } = profileData;
    return query(updateProfileQuery, [name, gender, phone, address, email])
...
module.exports.deleteAccount = function deleteAccount(userid, password) {
  ...
  return query(getPasswordQuery, [userid])
    ...
    .then(passwordMatch => {
      ...
      const deleteAccountQuery = 'DELETE FROM appuser WHERE userid = $1';
