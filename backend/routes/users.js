    ...
    userModel.checkExistingUser(email)
        .then(function (existingUser) {
            if (existingUser) {
                ...
                userModel.signup(name, email, password, phone, gender, address,region, role)
...
console.log(startDate,endDate)
        userModel.getTotalUserByAdmin(startDate, endDate)
            .then(users => {
                return res.json({ users });
            })
            .catch(error => {
...
router.get('/getUserCreationData', validationFn.validateToken, (req, res) => {
    try {
        ...
        userModel.getUserCreationData(startDate, endDate)
            .then(data => {
