...
router.post("/uploadProductPhoto", validationFn.validateToken, upload.single("photo"), (req, res) => {
    ...
    return imageModel.uploadCloudinaryPhotos(file)
        .then((result) => {
            var inputString = result.public_id;
            var imageId = inputString.split("/")[1];
            return imageModel.createProductImage(imageId, result.original_filename, result.url)
                .then((result1) => {
...
router.post("/createProductImage", validationFn.validateToken, function (req, res) {
    ...
    productsModel.createProductImage(productId, imageId)
        .then((result) => {
...
router.post('/productAdmin', validationFn.validateToken, function (req, res) {
    ...
    if (
        !Name || !Description || !unitPrice || !Category || !Color || !Object.keys(Color).length ||
        Object.values(Color).some(colorData =>
    ...
    return productsModel.getCategoryID(Category)
        .then(function (categoryID) {
            console.log(categoryID);
            // Call the model function to create the product
            return productsModel.createProductAndGetID({ productName: Name, description: Description, unitPrice, categoryID })
                .then(function (productID) {
                    ...
                    return Promise.all(selectColour)
                        .then((result) => {
...
router.put('/productDetailAdmin/:productdetailid/:qty', validationFn.validateToken, function (req, res) {
    ...
    return productsModel
        ...
        .then((count) => {
            if (count === 1) {
                return productsModel
                    .updateProductStatus(productdetailid)
                    .then(() => {
                        return res.json({ message: "Update Success" });
                    })
            }
...
router.delete('/deleteProduct', validationFn.validateToken, function (req, res) {
    ...
    return Promise.all([productsModel.deleteProductDetail(productid), imageModel.getImageIDByProductID(productid)])
        .then(([deleteCount, imageResult]) => {
            ...
            return Promise.all([productsModel.deleteProduct(productid), imageModel.deleteProductImage(productid)])
                .then(([deleteCount2, count]) => {
                    console.log(deleteCount2);
                    console.log(count);
                    if (imageResult && imageResult.imageid) {
                        return imageModel.deleteImage(imageResult.imageid)
                            .then(() => {
...
router.delete('/deleteProductDetail', validationFn.validateToken, function (req, res) {
    ...
    return productsModel.deleteProductDetailByID(productdetailid)
        .then((count) => {
            return res.status(201).json({ message: "Delete Success" });
        })
        .catch((error) => {
